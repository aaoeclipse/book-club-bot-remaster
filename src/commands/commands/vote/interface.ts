...
import { getVoteFromUserToBook, newUserVote, removeVote } from "./infrastructure"
...
export const voteForBook = async (interaction: ChatInputCommandInteraction) => {
    ...
    if (!currElection) {
        interaction.reply("No current elections in progress, create an election to add books to it")
        return
    }
    ...
    // Check if vote
    const voteExists = await getVoteFromUserToBook(userid, bookid, currElection.id)

    if (voteExists) {
        // removeVote
        const remd_book = await removeVote(voteExists.id)
        if (remd_book) {
